...
describe('QState', function () {
  ...
  describe('#eql', function () {
    ...
    it("will be false when one state has more non-zero amplitudes than the other", function () {
      ...
      expect(new Q('|00>').add(new Q('|01>'))).not.toEql(new Q('|00>'));
      expect(new Q('|00>')).not.toEql(new Q('|00>').add(new Q('|01>')));
    });
  ...
  describe('#controlledApplicatinOfqBitOperator', function () {
    it("does nothing when the control bit is zero (one target)", function () {
      ...
      var x = new Q('|001>').controlledApplicatinOfqBitOperator(2, 0, qbitFunction);
      expect(qbitFunction).not.toHaveBeenCalled();
      expect(x).toEql(new Q('|001>'));
    });
    it("does nothing when the control bit is zero (target range)", function () {
      var qbitFunction = jasmine.createSpy('qbitFunction');
      var targetBits = {from: 0, to: 1};
      var x = new Q('|001>').controlledApplicatinOfqBitOperator(2, targetBits, qbitFunction);
      expect(qbitFunction).not.toHaveBeenCalled();
      expect(x).toEql(new Q('|001>'));
    });
    it("does nothing when the control bit is zero (target array)", function () {
      var qbitFunction = jasmine.createSpy('qbitFunction');
      var x = new Q('|0001>').controlledApplicatinOfqBitOperator(3, [0, 2], qbitFunction);
      expect(qbitFunction).not.toHaveBeenCalled();
      expect(x).toEql(new Q('|0001>'));
    });
    ...
    it("flips the target bits when the control bit is one (target bit range)", function () {
      ...
      var qbitFunction = jasmine.createSpy('qbitFunction')
        .and.returnValue({amplitudeOf0: real(0.2), amplitudeOf1: real(0.3)});
      var state = new Q('|101>');
      var x = state.controlledApplicatinOfqBitOperator(2, targetBits, qbitFunction);
      expect(qbitFunction).toHaveBeenCalled();

      expect(qbitFunction.calls.all()[0].args).toEql([Complex.ZERO, Complex.ONE]);
      expect(qbitFunction.calls.all()[1].args).toEql([real(0.2), Complex.ZERO]);
      expect(qbitFunction.calls.all()[2].args).toEql([real(0.3), Complex.ZERO]);
      expect(x.amplitude('|100>')).toBeApprox(real(0.2));
    ...
    it("does nothing when any of the control bits are zero (control bit range)", function () {
      ...
      var x = new Q('|101>').controlledApplicatinOfqBitOperator(controlBits, 0, qbitFunction);
      expect(qbitFunction).not.toHaveBeenCalled();
      expect(x).toEql(new Q('|101>'));
    });
    ...
    it("invokes the qbitFunction when the control bits are all one (control bit range)", function () {
      ...
      expect(qbitFunction.calls.all()[0].args).toEql([Complex.ONE, Complex.ZERO]);
    ...
    it("throws an error when the control and target bits overlap", function () {
      var testFunction = function () {
        return {amplitudeOf0: real(0), amplitudeOf1: real(0)}
      };
  ...
  describe('#x', function () {
    it("applies the Pauli x operator to (|0>)", function () {
      var x = new Q('|0>').x(0);
      expect(x.amplitude('|0>')).toEql(Complex.ZERO);
      expect(x.amplitude('|1>')).toEql(Complex.ONE);
    });
    it("applies the Pauli x operator to (|1>)", function () {
      var x = new Q('|1>').x(0);
      expect(x.amplitude('|0>')).toEql(Complex.ONE);
      expect(x.amplitude('|1>')).toEql(Complex.ZERO);
    });
  });

  describe('#controlledX', function () {
    it("does nothing when the control bit is zero", function () {
      var x = new Q('|000>').controlledX(2, 0);
      expect(x.amplitude('|000>')).toBeApprox(Complex.ONE);
    });
    it("flips the target bit when the control bit is one", function () {
      var x = new Q('|100>').controlledX(2, 0);
      expect(x.amplitude('|101>')).toBeApprox(Complex.ONE);
    });
  });

  describe("#toffoli", function () {
    it("does nothing if any of the control bits are zero", function () {
      var x = new Q('|0010>').toffoli(3, 1, 0);
      expect(x.amplitude('|0010>')).toBeApprox(Complex.ONE);
    });
    it("flips the target bit when all of the control bits are one", function () {
      var x = new Q('|1010>').toffoli(3, 1, 0);
      expect(x.amplitude('|1011>')).toBeApprox(Complex.ONE);
    });
  });
  ...
  describe('#controlledZ', function () {
    ...
    it("does nothing when an even number of the target bits are 1 when the control bit is one", function () {
      var targetBits = {from: 0, to: 1};
      var x = new Q('|111>').controlledZ(2, targetBits);
      expect(x.amplitude('|111>')).toBeApprox(Complex.ONE);
    });
  });
  ...
  describe('#controlledT', function () {
    ...
    it("applies the T operator when the control bit is one (target is 0)", function () {
      var x = new Q('|001>').controlledT(0, 2);
      expect(x).toEql(new Q('|001>'));
    });
  ...
  describe('#hadamard', function () {
    it("applies the hadamard operation", function () {
      ...
      expect(x.amplitude('|001>')).toEql(Complex.ZERO);
      expect(x.amplitude('|010>')).toEql(Complex.ZERO);
      expect(x.amplitude('|011>')).toEql(Complex.ZERO);
      expect(x.amplitude('|100>')).toBeApprox(real(1 / Math.sqrt(2)));
      expect(x.amplitude('|101>')).toEql(Complex.ZERO);
      expect(x.amplitude('|110>')).toEql(Complex.ZERO);
      expect(x.amplitude('|111>')).toEql(Complex.ZERO);
    });

    it("is it's own inverse", function () {
      var x = new Q('|000>').hadamard(2).hadamard(2);
      expect(x.amplitude('|000>')).toBeApprox(Complex.ONE);
      expect(x.amplitude('|001>')).toBe(Complex.ZERO);
      expect(x.amplitude('|010>')).toBe(Complex.ZERO);
      expect(x.amplitude('|011>')).toBe(Complex.ZERO);
      expect(x.amplitude('|100>')).toBe(Complex.ZERO);
      expect(x.amplitude('|101>')).toBe(Complex.ZERO);
      expect(x.amplitude('|110>')).toBe(Complex.ZERO);
      expect(x.amplitude('|111>')).toBe(Complex.ZERO);
    });

    it("accepts an ALL parameter", function () {
      var x = new Q('|00>').hadamard(Q.ALL);
    ...
    it("accepts a bit range", function () {
      var x = new Q('|000>').hadamard({from: 1, to: 2});
  ...
  describe('#controlledHadamard', function () {
    it("does nothing when the control bit is zero", function () {
      var x = new Q('|001>').controlledHadamard(2, 0);
      expect(x).toEql(new Q('|001>'));
    });
    it("applies the Hadamard operator when the control bits are one", function () {
      var x = new Q('|111>').controlledHadamard({from: 1, to: 2}, 0);
  ...
  describe('#rotateX', function () {
    it("rotates about the X axis", function () {
      var x = new Q('|00>').rotateX(1, Math.PI / 4);

      expect(x.amplitude('|00>')).toBeApprox(real(Math.cos(Math.PI / 8)));
      expect(x.amplitude('|01>')).toBe(Complex.ZERO);
      expect(x.amplitude('|10>')).toBeApprox(complex(0, -Math.sin(Math.PI / 8)));
      expect(x.amplitude('|11>')).toBe(Complex.ZERO);
    });
    it("can be applied multiple times", function () {
      var x = new Q('|00>').rotateX(1, Math.PI / 4).rotateX(1, Math.PI / 4).rotateX(1, Math.PI / 4);
    ...
    it("is accepts an ALL parameter", function () {
      var x = new Q('|00>').rotateX(Q.ALL, Math.PI / 4);
  ...
  describe('#controlledXRotation', function () {
    it("does nothing when the control bit is zero", function () {
      var x = new Q('|001>').controlledXRotation(2, 0, Math.PI / 4);
      expect(x).toEql(new Q('|001>'));
    });
    it("rotates around the x axis when the control bit is one", function () {
      var x = new Q('|100>').controlledXRotation(2, 0, Math.PI / 4);
  ...
  describe('#rotateY', function () {
    it("rotates about the Y axis", function () {
      var x = new Q('|00>').rotateY(1, Math.PI / 4);
      expect(x.amplitude('|00>')).toBeApprox(real(Math.cos(Math.PI / 8)));
      expect(x.amplitude('|01>')).toBe(Complex.ZERO);
    ...
    it("can be applied multiple times", function () {
      var x = new Q('|00>').rotateY(1, Math.PI / 4).rotateY(1, Math.PI / 4).rotateY(1, Math.PI / 4);
      expect(x.amplitude('|00>')).toBeApprox(real(Math.cos(3 * Math.PI / 8)));
      expect(x.amplitude('|01>')).toBe(Complex.ZERO);
  ...
  describe('#controlledYRotation', function () {
    it("does nothing when the control bit is zero", function () {
      var x = new Q('|001>').controlledYRotation(2, 0, Math.PI / 4);
      expect(x).toEql(new Q('|001>'));
    });
    it("rotates around the y axis when the control bit is one", function () {
      var x = new Q('|100>').controlledYRotation(2, 0, Math.PI / 4);
      expect(x.amplitude('|100>')).toBeApprox(real(Math.cos(Math.PI / 8)));
  ...
  describe('#rotateZ', function () {
    it("rotates about the Z axis (|0>)", function () {
      var x = new Q('|0>').rotateZ(0, Math.PI / 4);
      expect(x.amplitude('|0>')).toBeApprox(complex(Math.cos(Math.PI / 8), -Math.sin(Math.PI / 8)));
      expect(x.amplitude('|1>')).toBe(Complex.ZERO);
    });
    it("rotates about the Z axis (|1>)", function () {
      var x = new Q('|1>').rotateZ(0, Math.PI / 4);
      expect(x.amplitude('|0>')).toBe(Complex.ZERO);
      expect(x.amplitude('|1>')).toBeApprox(complex(Math.cos(Math.PI / 8), Math.sin(Math.PI / 8)));
    });
    it("can be applied multiple times", function () {
      var x = new Q('|0>').rotateZ(0, Math.PI / 4).rotateZ(0, Math.PI / 4).rotateZ(0, Math.PI / 4);
  ...
  describe('#controlledZRotation', function () {
    it("does nothing when the control bit is zero", function () {
      var x = new Q('|001>').controlledZRotation(2, 0, Math.PI / 4);
      expect(x).toEql(new Q('|001>'));
    });
    it("rotates around the z axis when the control bit is one", function () {
      var x = new Q('|100>').controlledZRotation(2, 0, Math.PI / 4);
  ...
  describe('#controlledR', function () {
    it("does nothing when the control bit is zero", function () {
      var originalState = new Q('|000>').hadamard(0);
    ...
    it("shifts the phase by e^(i angle) when the control bit is one", function () {
      var originalState = new Q('|100>').hadamard(0);
      var x = originalState.controlledR(2, 0, Math.PI / 4);
  ...
  describe('#r', function () {
    it("shifts the phase by the specified angle e^(i angle)", function () {
      var originalState = new Q('|0>').hadamard(0);
  ...
  describe('Simple combination of hadamard and cnot', function () {
    it("results in a phase kick back", function () {
      var x = new Q('|01>').hadamard(0).hadamard(1).cnot(1, 0).hadamard(0).hadamard(1);
      expect(x.amplitude('|00>')).toBe(Complex.ZERO);
      expect(x.amplitude('|01>')).toBe(Complex.ZERO);
      expect(x.amplitude('|10>')).toBe(Complex.ZERO);
      expect(x.amplitude('|11>')).toBeApprox(Complex.ONE);
    });
  });
  describe('#applyFunction', function () {
    ...
    it("flips the target bit from one to zero when the function returns one", function () {
      ...
      var x = new Q('|01>').applyFunction(1, 0, f);
      expect(x).toEql(new Q('|00>'));
    });
    ...
    it("can flip multiple target bits", function () {
      ...
      var x = new Q('|1011>').applyFunction(3, {from: 0, to: 2}, f);
      expect(x).toEql(new Q('|1110>'));
    });

    it("restricts flipping of target bits to those specified", function () {
      var f = function (x) {
        return parseInt('1101', 2);
      };
      var x = new Q('|1011>').applyFunction(3, {from: 0, to: 2}, f);
      expect(x).toEql(new Q('|1110>'));
    });
    it("throws exception when target and control bits overlap", function () {
      var badFunctionInvocation = function () {
        new Q("0000").applyFunction({from: 0, to: 2}, {from: 2, to: 3}, function (x) {
          return x;
        });
      };
      expect(badFunctionInvocation).toThrow("control and target bits must not be the same nor overlap");
  ...
  describe('#each', function () {
    it('should invoke a callback with a StateWithAmplitude', function () {
      ...
      var stateWithAmplitude0 = callBack.calls.all()[0].args[0];
      var stateWithAmplitude2 = callBack.calls.all()[1].args[0];
    ...
    it('should break early when returned false', function () {
      var callCount = 0;
      new Q('|10>').hadamard(Q.ALL).each(function (stateWithAmplitude) {
        callCount++;
...
//            0.5 |1000> + 0.5 |1001> + 0.5 |1100> + 0.5 |1101>
      stateToMeasure = new Q('|1000>').hadamard(2).hadamard(0);
    });

    it("should return the new states for outcome of 00 (random returns 0)", function () {
      stateToMeasure.random = function () {
        return 0
      };
      ...
      expect(newState.amplitude('|1100>')).toBe(Complex.ZERO);
      expect(newState.amplitude('|1101>')).toBe(Complex.ZERO);
    });
    ...
    it("should return the new states for outcome of 10 (random returns 0.51)", function () {
      ...
      expect(newState.amplitude('|1000>')).toBe(Complex.ZERO);
      expect(newState.amplitude('|1001>')).toBe(Complex.ZERO);
    ...
    it("Can measure all bits", function () {
      ...
      var measurement = stateToMeasure.measure(Q.ALL);
      var newState = measurement.newState;
      expect(newState.numBits()).toBe(4);
      expect(measurement.result).toBe(parseInt('1001', 2));
      expect(newState.amplitude('|1000>')).toBe(Complex.ZERO);
      expect(newState.amplitude('|1001>')).toBeApprox(Complex.ONE);
      expect(newState.amplitude('|1100>')).toBe(Complex.ZERO);
      expect(newState.amplitude('|1101>')).toBe(Complex.ZERO);
    });
  ...
  describe('Measurement', function () {
    it('should have a asBitString() function', function () {
      expect(new Q('|0101>').measure({from: 1, to: 3}).asBitString()).toBe("010");
    });
  });

  describe('#kron', function () {
    it('should return the tensor product of two states', function () {
      var q1 = new Q('|01>').hadamard(0);
      /* sqrt(1/2)(|00> - |01> */
      var q2 = new Q('|100>').hadamard(2);
      /* sqrt(1/2) |000> - |100> */
      var newState = q1.kron(q2);
      /* Should be 0.5 (|00000> - |00100> - |01000> + |01100> */
  ...
  describe("#tensorProduct", function () {
    it("should be an alias for #kron", function () {
      ...
      var q2 = new Q('|100>').hadamard(2);
      /* sqrt(1/2) |000> - |100> */
      var newState = q1.tensorProduct(q2);
      /* Should be 0.5 (|00000> - |00100> - |01000> + |01100> */
      expect(newState.amplitude('|00000>')).toBeApprox(real(0.5));
      expect(newState.amplitude('|00100>')).toBeApprox(real(-0.5));
      expect(newState.amplitude('|01000>')).toBeApprox(real(-0.5));
  ...
  describe("#multiply", function () {
    it("should modify the global phase", function () {
      var q = new Q('|1>').hadamard(0).multiply(complex(3, -4));
    ...
    it("should accept non-complex numbers", function () {
      var q = new Q('|1>').hadamard(0).multiply(2);
...
//            NOTE: You will need to normalize afterwards to get a sensible answer!
      var q = new Q('|01>').hadamard(0).add(new Q('|00>')).add(new Q('|11>'));
...
//            NOTE: You will need to normalize afterwards to get a sensible answer!
      var q = new Q('|01>').hadamard(0).add(new Q('|00>')).subtract(new Q('|01>'));
  ...
  describe("#normalize", function () {
    it("should normalize the amplitudes", function () {
      var q = new Q('|0>')
        ...
        .add(new Q('|1>').multiply(complex(0, 1)));
      q = q.normalize();
