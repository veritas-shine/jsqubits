...
function dqc1(f, n) {
    ...
    return new Q.Complex(real, imaginary);
}
...
function traceComponent(f, n, rotation) {
    ...
    for (var i = 0; i < numberOfSamples; i++) {
        var amplitudes = {};
        amplitudes[Math.floor(Math.random() * numberOfMixedStates)] = new Q.Complex(1,0);
        var state = new Q.QState(n+1, amplitudes).hadamard(n);
        state = f(state);
